...
const FIELDS: Field[] = [
  ...
  { key: "payment_iban", label: "IBAN", value: "ES•• •••• •••• •••• •••• 0294", x: 65.3, y: 61.6, w: 10.0, h: 1.8, size: 0.87, color: "#202124" },
  { key: "payment_name", label: "Nombre titular", value: "Joel pedroche graciano", x: 65.3, y: 64.0, w: 9.0, h: 2.2, size: 0.78, color: "#5f6368" },
...
export default function AdsenseClient({ initialOverrides }: { initialOverrides: Record<string, string> }) {
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", overflow: "hidden", position: "relative" }}>
      ...
      {FIELDS.map(f => {
        ...
        // adminonly fields: only render text overlay in admin mode
        if (f.bg === "adminonly" && !isAdmin && !overrides[f.key]) return null;
        return (
          <div
            key={`text-${f.key}`}
            style={{
              ...
              width: `${f.w}%`,
              height: `${f.h}%`,
              backgroundColor: (f.bg === "none" || f.bg === "adminonly") ? "transparent" : (f.bg || "transparent"),
              display: "flex",
              alignItems: "center",
